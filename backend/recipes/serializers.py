...
class CreateRecipeSerializer(serializers.ModelSerializer):
    image = Base64ImageField(max_length=200, use_url=True)
    author = UserSerializerModified(read_only=True)
    ...
    def create(self, validated_data):
        ...
        for tag in tags_data:
            ReceiptTags.objects.create(recipe=recipe, tag=tag)
        return recipe

    def update(self, instance, validated_data):
        tags_data = validated_data.pop('tags')
        ingredient_data = validated_data.pop('ingredients')
        ReceiptTags.objects.filter(recipe=instance).delete()
        for tag in tags_data:
            ReceiptTags.objects.create(
                recipe=instance,
                tag=tag
            )
